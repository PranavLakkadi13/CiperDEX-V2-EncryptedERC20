...
interface EncryptedERC20 {
    ...
    function mint(uint32 mintedAmount) external;

    // Transfers an encrypted amount from the message sender address to the `to` address.
    function transfer(address to, inEuint32 calldata encryptedAmount) external returns (bool);
    ...
    function approve(address spender, euint32 amount) external returns (bool) ;
